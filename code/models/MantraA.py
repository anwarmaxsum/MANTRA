...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        print("num_learners: " + str(self.num_models))
        ...
        # self.reshapeTensor.requires_grad=True
        # self.reshapelayer = ReshapeLayer(3)
    ...
    def forward_for_slowlearner(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        # dec_out = self.reshapelayer(dec_out)
        # dec_out = torch.matmul(self.reshapeTensor,dec_out)
...
class ReshapeLayer(nn.Module):
    def __init__(self, n_learner):
        super(ReshapeLayer, self).__init__()
        self.reshapeTensor = torch.rand((1, n_learner), requires_grad=True).cuda()
        # self.reshapeTensor = F.softmax(self.reshapeTensor,dim=1)
        # self.reshapeTensor.requires_grad=True

    def forward(self, input):
        return torch.einsum("ij, jklm -> iklm", self.reshapeTensor, input)


    def check_params(self):
        print(self.reshapeTensor)