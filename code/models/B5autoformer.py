...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if self.output_attention:
            ...
            # d0 = self.models[0].forward(x_enc[:16,:,:], x_mark_enc[:16,:,:], x_dec[:16,:,:], x_mark_dec[:16,:,:], enc_self_mask, dec_self_mask, dec_enc_mask)
            # d1 = self.models[1].forward(x_enc[16:32,:,:], x_mark_enc[16:32,:,:], x_dec[16:32,:,:], x_mark_dec[16:32,:,:], enc_self_mask, dec_self_mask, dec_enc_mask)
            # print(d0.shape)
            # print(d1.shape)
            
            dec_out=[]
            offs = int(math.ceil(x_enc.shape[0]/self.num_models))
            for i in range (0, self.num_models):
                ...
                do = self.models[i].forward(x_enc[ist:fin,:,:], x_mark_enc[ist:fin,:,:], x_dec[ist:fin,:,:], x_mark_dec[ist:fin,:,:], enc_self_mask, dec_self_mask, dec_enc_mask)

                if (i==0):
                    dec_out = do
                else:
                    dec_out = torch.cat((dec_out,do),dim=0)
            # dec_out = torch.stack(dec_out)


            # dec_out = torch.cat((d0,d1),dim=0)
            return dec_out
