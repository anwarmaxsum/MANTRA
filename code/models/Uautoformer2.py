...
from .new_prop_prototype2 import MultiHeadURT, SingleHeadMVURT
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # self.URT_model  = MultiHeadURT(key_dim=self.flat_dim, query_dim=configs.batch_size*self.flat_dim, hid_dim=1024, temp=1, att="cosine", n_head=1)
        # self.URT_model  = MultiHeadURT(key_dim=configs.pred_len , query_dim=configs.pred_len*configs.enc_in, hid_dim=1024, temp=1, att="cosine", n_head=1)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        for i in range(0,d):
            for j in range(0,a):
                ...
                fin_out[:,:,i] = fin_out[:,:,i] + dec_out[j,:,:,i] * urt_out[j]
