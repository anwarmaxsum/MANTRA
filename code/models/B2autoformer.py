...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        for i in range (0, self.num_models):
            ...
            enc_out, attns_i = self.encoder[i](enc_out, attn_mask=enc_self_mask)

            dec_out_i = self.dec_embedding(seasonal_init, x_mark_dec)
            seasonal_part, trend_part = self.decoder[i](dec_out_i, enc_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask,
                                                     trend=trend_init)
            # final
            dec_out_i = trend_part + seasonal_part
