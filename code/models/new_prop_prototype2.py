...
class URTPropagation(nn.Module):
  ...
  def forward(self, cat_proto):
    ...
    # score   = F.softmax(self.temp * raw_score, dim=1)
    score = torch.mean(self.temp * raw_score, dim=1)
...
#Modification with modifying utput
# class URTPropagation(nn.Module):

#   def __init__(self, key_dim, query_dim, hid_dim,  pred_length, temp=1, att="cosine"):
#     super(URTPropagation, self).__init__()
#     # self.linear_qt = nn.Linear(1, 24, bias=True)
#     # self.linear_kt = nn.Linear(1, 24, bias=True)
#     self.pred_length = pred_length
#     self.linear_qt = nn.Linear(1, pred_length, bias=True)
#     self.linear_kt = nn.Linear(1, pred_length, bias=True)
#     self.linear_q = nn.Linear(query_dim, hid_dim, bias=True)
#     self.linear_k = nn.Linear(key_dim, hid_dim, bias=True)
#     #self.linear_v_w = nn.Parameter(torch.rand(8, key_dim, key_dim))
#     self.linear_v_w = nn.Parameter( torch.eye(key_dim).unsqueeze(0).repeat(8,1,1)) 
...
#     # cat_proto n_class*8*512 
#     # return: n_class*8
#     # print("cat_proto.shape")
#     # print(cat_proto.shape)
#     n_class, n_extractors, fea_dim = cat_proto.shape
#     # q       = cat_proto.view(n_class, -1) # n_class * 8_512
#     # q       = cat_proto.reshape(n_class, -1)
#     # k       = cat_proto                   # n_class * 8 * 512

#     q       = cat_proto.reshape(n_class, n_extractors*fea_dim, 1)
#     k       = cat_proto.reshape(n_class, n_extractors, fea_dim, 1)                  # n_class * 8 * 512=
#     # print("q and k")
#     # print(q.shape)
#     # print(k.shape)
...
#     q = self.linear_qt(q)
#     d1, d2, d3 = q.shape
#     q = q.reshape(d1,d3,d2)
#     k = self.linear_kt(k)
#     # print("q and k after transformed")
#     # print(q.shape)
#     # print(k.shape)
...
#     if self.att == "cosine":
#       # print("use cosine")
#       # print(q_emb.view(n_class, 1, -1).shape)
#       # print(k_emb.view(n_class, n_extractors, -1).shape)
#       raw_score   = F.cosine_similarity(q_emb.view(n_class, 1, -1), k_emb.view(n_class, n_extractors, -1), dim=-1)
#     elif self.att == "dotproduct":
#       # print("use dot product")
#       # print(q_emb.view(n_class, 1, self.pred_length, -1).shape)
#       # print(k_emb.view(n_class, n_extractors, self.pred_length, -1).shape)
#       raw_score   = torch.sum( q_emb.view(n_class, 1, self.pred_length, -1) * k_emb.view(n_class, n_extractors, self.pred_length, -1), dim=-1 ) / (math.sqrt(fea_dim)) 
#     else:
#       # print("use else function")
#       raise ValueError('invalid att type : {:}'.format(self.att))
    
#     # score   = F.softmax(self.temp * raw_score, dim=1)
#     score   =  self.temp * raw_score
#     # print("raw score: " + str(raw_score))
#     # print("raw score shape:")
...
# Modfication with modifying output
# class MultiHeadURT(nn.Module):
#   def __init__(self, key_dim, query_dim, hid_dim, pred_length, temp=1,  att="cosine", n_head=1):
#     super(MultiHeadURT, self).__init__()
#     layers = []
#     for _ in range(n_head):
#       layer = URTPropagation(key_dim, query_dim, hid_dim, pred_length, temp,  att)
#       layers.append(layer)
#     self.layers = nn.ModuleList(layers)
...
class SingleHeadMVURT(nn.Module):
  def __init__(self, key_dim, query_dim, hid_dim, pred_length, temp=1,  att="cosine", nv=1):
    super(SingleHeadMVURT, self).__init__()
    layers = []
    for _ in range(nv):
      layer = URTPropagation(key_dim, query_dim, hid_dim, pred_length, temp,  att)
      layers.append(layer)
